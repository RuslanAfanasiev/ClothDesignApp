          ...
          <View style={styles.card}>
            ...
            {step === 0 && (
              <>
                ...
                >
                  <LinearGradient
                    colors={email.trim() ? ['#E8CC6A', '#D4AF37'] : ['#2A2A38', '#2A2A38']}
                    start={{ x: 0, y: 0 }}
                    end={{ x: 1, y: 0 }}
                    style={styles.btn}
                  >
                    {loading
                      ...
                      : <Text style={[styles.btnText, !email.trim() && styles.btnTextDisabled]}>
                          Send Code
                        </Text>
                    }
                  </LinearGradient>
            ...
            {step === 1 && (
              <>
                ...
                <Text style={styles.cardSub}>Enter the 6-digit code we sent to{'\n'}{email}</Text>
                <View style={styles.divider} />
                ...
                >
                  <LinearGradient
                    colors={isOtpFilled ? ['#E8CC6A', '#D4AF37'] : ['#2A2A38', '#2A2A38']}
                    start={{ x: 0, y: 0 }}
                    end={{ x: 1, y: 0 }}
                    style={styles.btn}
                  >
                    <Text style={[styles.btnText, !isOtpFilled && styles.btnTextDisabled]}>
                      Verify Code
                    </Text>
                  </LinearGradient>
                </TouchableOpacity>
                <TouchableOpacity onPress={() => setStep(0)} style={styles.backInCard}>
            ...
            {step === 2 && (
              <>
                ...
                >
                  <LinearGradient
                    colors={newPassword.trim() ? ['#E8CC6A', '#D4AF37'] : ['#2A2A38', '#2A2A38']}
                    start={{ x: 0, y: 0 }}
                    end={{ x: 1, y: 0 }}
                    style={styles.btn}
                  >
                    {loading
                      ? <ActivityIndicator color="#0B0B0F" />
                      : <Text style={[styles.btnText, !newPassword.trim() && styles.btnTextDisabled]}>
                          Reset Password
                        </Text>
                    }
                  </LinearGradient>
...
const styles = StyleSheet.create({
  ...
  brandTitle: { fontSize: 12, fontWeight: '700', letterSpacing: 4, color: '#D4AF37' },
  ...
  stepCircle: {
    ...
    borderWidth: 1.5, borderColor: '#2A2A38',
    backgroundColor: '#13131A',
    alignItems: 'center', justifyContent: 'center',
  },
  stepCircleActive: { borderColor: '#D4AF37', backgroundColor: 'rgba(212,175,55,0.1)' },
  ...
  stepLabel: { fontSize: 9, letterSpacing: 0.8, color: '#4A4A5A', textTransform: 'uppercase' },
  stepLabelActive: { color: '#D4AF37' },
  stepLine: { flex: 1, height: 1.5, backgroundColor: '#2A2A38', marginHorizontal: 6, marginBottom: 16 },
  ...
  cardSub: { fontSize: 13, color: '#6B6B80', letterSpacing: 0.2, lineHeight: 20 },
  divider: { height: 1, backgroundColor: '#2A2A38', marginVertical: 20 },

  fieldLabel: { fontSize: 10, letterSpacing: 1.4, textTransform: 'uppercase', color: '#6B6B80', marginBottom: 6 },
  input: {
  ...
  otpBoxFilled: { borderColor: '#D4AF37', backgroundColor: 'rgba(212,175,55,0.08)' },
